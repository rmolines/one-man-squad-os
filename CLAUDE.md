...
| `.sheet` no macOS — sem light-dismiss | `.sheet` é modal nativo no macOS; clicar fora não fecha o painel — comportamento esperado da plataforma, mas surpreendente vindo do iOS | Usar ZStack overlay no pai: `Color.black.opacity(0.001).onTapGesture { show = false }` como backdrop clicável atrás do painel |
...
| `gh` em worktree multi-remote | `gh pr create` detecta `upstream` (claude-kickstart) em vez de `origin` quando ambos estão configurados — falha com "Head sha can't be blank" | Sempre usar `gh pr create -R rmolines/one-man-squad-os` (flag `-R` explícita) em todas as chamadas `gh` neste projeto |
| `private struct` em View — bloqueio de reuso | Componente auxiliar definido como `private struct` (ex: `PhaseChip`, `StatusChip`) em um arquivo de View não pode ser usado em Views irmãs do mesmo target — gera duplicação de código | Usar `internal struct` (sem modificador) quando o componente será compartilhado entre arquivos do mesmo target; `private` só quando o uso é estritamente local ao arquivo |
| `var` computed property em View — re-parse por render | `private var sections: ParsedSections { ParsedSections(markdown: raw) }` re-executa o parse a cada render do SwiftUI; quando o input é um `let` imutável, o trabalho é desperdiçado em cada frame | Quando o input não muda após a construção, usar `let sections: ParsedSections` inicializado no `init` da View — o parse ocorre uma única vez; não confundir com o pitfall `@Observable + let in init` (que quebra reatividade — esse caso é input estático) |
| `git push` — HTTP 400 em pack grande | O `http.postBuffer` padrão do git é pequeno demais para pushes com diffs binários grandes (ex: mudanças em xcodeproj) — falha silenciosa com HTTP 400 | `git config http.postBuffer 524288000` (global ou por repo); equivale a 500 MB de buffer |
| Squash-merge + rebase no mesmo worktree | Após um squash-merge, fazer `git rebase origin/main` no branch original do worktree falha com conflitos: git vê cada commit original colidindo com o commit squash já presente em main | Criar um branch limpo a partir de `origin/main` (`git checkout -b feat/X origin/main`) e cherry-pick apenas os commits novos que ainda não foram mergeados; não rebaser o branch original |
| `PortfolioStore.refresh()` em onSave/onWrite | Chamar `refresh(repoPath:)` após um write in-app causa double-reload: FSEvents detecta o write atômico ~1s depois e dispara `reload()` independentemente — PortfolioView pisca `isLoading` duas vezes | Usar `reloadNow()` para writes in-app; reservar `refresh(repoPath:)` para mudança de repo ou startup |
